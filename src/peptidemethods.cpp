...
double PeptideMethods::calculateRTBioLCCC(
        ...
        if ( j > currentGradientPoint->first ) {
            ...
            if ( currentGradientPoint != --convertedGradient.end() ) {
                ...
                if (currentGradientPoint->second == 
                    ...
                    if (((1 - S) / dV * 
                        calculateKdBioLCCC(peptideEnergyProfile, chemBasis, 
                    ...
                    {
                        j += ceil((1 - S) / dV * 
                                 calculateKdBioLCCC(peptideEnergyProfile, 
                                     chemBasis,
                                     conditions.secondSolvent(), 
                                     secondSolventConcentration,
                                     conditions.columnPoreSize(),
                    ...
                    // a peptide to elute.
                        ...
                        S += dV / calculateKdBioLCCC(peptideEnergyProfile,
                                      chemBasis,
                                      conditions.secondSolvent(), 
                                      secondSolventConcentration, 
                                      conditions.columnPoreSize(), 
    ...
    RT += j * dV / conditions.flowRate() + conditions.delayTime() + 
...
double PeptideMethods::calculateKdBioLCCC(const std::string &sequence,
    ...
    std::vector<Aminoacid> parsedPeptideStructure;
    Terminus NTerminus;    Terminus CTerminus;
    std::vector<double> peptideEnergyProfile;
...
double PeptideMethods::calculateKdBioLCCC(
    const std::vector<double> &peptideEnergyProfile,
    const ChemicalBasis &chemBasis,
    const std::string secondSolvent,
    const double secondSolventConcentration,
...
bool PeptideMethods::parseSequence(
    ...
    *NTerminus = chemBasis.defaultNTerminus()->second ;
    for (std::map<std::string,Terminus>::const_iterator NTermIterator = 
            chemBasis.beginNTerminus(); 
        NTermIterator != chemBasis.endNTerminus(); 
        NTermIterator++
    ) {
        if (strippedSource.find(NTermIterator->second.label()) != 
                std::string::npos) {
    ...
    *CTerminus = chemBasis.defaultCTerminus()->second;
    ...
    if (CTerminusPosition != std::string::npos){
        ...
        for (std::map<std::string,Terminus>::const_iterator CTermIterator = 
                 chemBasis.beginCTerminus(); 
             CTermIterator != chemBasis.endCTerminus(); 
             CTermIterator++
        ) {
            if (strippedSource.find(CTermIterator->second.label(), 
    ...
    while (curPos < strippedSource.size()) {
        ...
        for (std::map<std::string,Aminoacid>::const_iterator currentAminoacid =
                 chemBasis.beginAminoacid(); 
             currentAminoacid != chemBasis.endAminoacid(); 
             currentAminoacid++
        ) {
            if (strippedSource.compare(curPos, 
                    currentAminoacid->second.label().size(), 
                    currentAminoacid->second.label()) == 0) {
...
bool PeptideMethods::calculatePeptideProperties(const std::string &sequence,
   ...
   if ( parseSequence(sequence, 
                    ...
                    &peptideEnergyProfile) )
    {
        *RTBioLCCC = calculateRTBioLCCC(peptideEnergyProfile, chemBasis, 
                                        conditions);
